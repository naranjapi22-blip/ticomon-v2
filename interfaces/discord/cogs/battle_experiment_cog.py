...
class BattleExperimentCog(commands.Cog):
    def __init__(self, core) -> None:
        self._core = core

    @commands.command(name="batalla")
    async def batalla(
        self,
        ...
        message = await ctx.send(
            embed=view.build_embed(battle),
            view=view,
        )
        view.message = message

    @batalla.error
    async def batalla_error(
        self,
        ctx: commands.Context,
        ...
        if isinstance(error, commands.MissingRequiredArgument):
            if error.param.name == "opponent":
                await ctx.send("Usage: `!batalla @trainer`")
                return
