...
class PvpEventTranslator:
    ...
    def _translate_event(event: str, values: list[str]) -> str | None:
        if event == "move" and len(values) >= 2:
            return f"{_pokemon_name(values[0])} used {values[1]}."
        if event == "-damage" and len(values) >= 2:
            return f"{_pokemon_name(values[0])} lost {values[1]} HP."
        ...
        if event == "-supereffective" and values:
            return f"It was super effective against {_pokemon_name(values[0])}."
        if event == "-resisted" and values:
            return f"It was not very effective against {_pokemon_name(values[0])}."
        ...
        if event == "switch" and len(values) >= 2:
            return f"{_pokemon_name(values[0])} sent out {values[1]}."
