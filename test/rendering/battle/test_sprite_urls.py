...
def test_battle_initiator_sprite_urls():
    assert (
        battle_initiator_sprite_url(25, shiny=False)
        == f"{BASE_GIF_URL}/back/25"
    )
    assert (
        battle_initiator_sprite_url(25, shiny=True)
        == f"{BASE_GIF_URL}/back_shiny/25"
    )


def test_battle_opponent_sprite_urls():
    assert (
        battle_opponent_sprite_url(7, shiny=False)
        == f"{BASE_GIF_URL}/regular/7"
    )
    assert (
        battle_opponent_sprite_url(7, shiny=True)
        == f"{BASE_GIF_URL}/shiny/7"
    )
